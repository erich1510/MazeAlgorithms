...
public class MazeGraph {
    ...
    */
     ...
     */
        ...
        while (frontier.size() != 0){
            ...
            if (currVertex == processedGraph.getGoalNode()){
                return ReconstructPath(preds, processedGraph.getStartNode(), processedGraph.getGoalNode());
            }
            else{
                ...
                for (Integer n : neighs) {
                    if (!visited.contains(n)) {
                        ...
                        if (n == processedGraph.getGoalNode()){
                            ReconstructPath(preds,processedGraph.getStartNode(),processedGraph.getGoalNode());
                        }
                        visited.add(n);
                        frontier.add(n);
                    }
    ...
    private static ArrayList<Integer> DFSRecur (ProcessedGraph processedGraph, HashMap<Integer, Integer> preds, Integer
    ...
    public static ArrayList<Integer> ReconstructPath(HashMap<Integer, Integer> preds, Integer start, Integer goal) {
        Integer currNode = goal;
        ArrayList<Integer> path = new ArrayList<>();
        while (currNode != start) {
            path.add(0,currNode);
            currNode = preds.get(currNode);
        }
        path.add(0,start);
        return path;
    }
